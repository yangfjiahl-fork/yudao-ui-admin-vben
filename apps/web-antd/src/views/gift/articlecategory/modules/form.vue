...
const formData = ref<GiftArticleCategoryApi.ArticleCategory>();
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    const data =
      (await formApi.getValues()) as GiftArticleCategoryApi.ArticleCategory;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<GiftArticleCategoryApi.ArticleCategory>();
    if (!data || !data.id) {
      // 设置上级
